...
$config = array('hostname' => 'localhost', 'username' => 'root', 'password' => '', 'dbname' => 'chatdb');
...
function Send()
{
    ...
    $mail = substr($_POST['mail'], 0, 200); // обрезаем до 200 символов
    $mail = htmlspecialchars($mail); // заменяем опасные теги (<h1>,<br>, и прочие) на безопасные
    $mail = mysql_real_escape_string($mail); // функция экранирует все спец-символы в unescaped_string , вследствие чего, её можно безопасно использовать в mysql_query()
    // добавляем новую запись в таблицу messages
    mysql_query("INSERT INTO messages (name,text,mail) VALUES ('" . $name . "', '" . $text . "','" . $mail . "')");
}
...
function Load()
{
    ...
    $query = mysql_query("SELECT * FROM messages  ORDER BY id DESC LIMIT 10");

    // проверяем есть ли какие-нибудь новые сообщения
    if (mysql_num_rows($query) > 0) {
        // начинаем формировать javascript который мы передадим клиенту
        $js = 'var chat = $("#mess_area");'; // получаем "указатель" на div, в который мы добавим новые сообщения
        ...
        foreach ($messages as $value) {
            ...
            $js .= 'chat.append("<span>' . $value['name'] . '&raquo; ' . $value['text'] .'&raquo; ' . $value['mail'] .'</span>");'; // добавить сообщние (<span>Имя &raquo; текст сообщения</span>) в наш div
        }
